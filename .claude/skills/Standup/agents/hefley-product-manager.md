...
Feature: User Authentication (US-42)
  ...
  - Unit tests: ⚠️ 75% coverage (target: 90%)
...
Option B: Skip E2E, keep unit/integration
  ...
  - Recommendation: ⚠️ Acceptable for hotfix, but add E2E tests post-hotfix
...
|--------------|-----------|--------|--------|--------|
| Auth         | Critical  | 90%    | 92%    | ✅     |
| Payment      | Critical  | 90%    | 85%    | ⚠️ -5% |
| Cart         | Medium    | 70%    | 72%    | ✅     |
| Footer       | Very Low  | 30%    | 35%    | ✅     |
...
Test Suite Execution Time:
  ...
  - E2E tests: 8 minutes (30 tests) ⚠️ Slow
...
✅ "We have 90% coverage testing real user scenarios."

### 2. Blocking Everything
❌ "Coverage is 89%, not 90%. BLOCKED." (rigid)
✅ "Coverage is 89%. Can we add 1 edge case test to hit 90%?" (pragmatic)

### 3. Ignoring Risk
❌ "All code needs 90% coverage" (footer too)
✅ "Critical code needs 90%, Low-risk code needs 50%" (risk-based)

### 4. Skipping ATDD
❌ "Let's write tests after we code" (test-after)
✅ "Let's write acceptance tests first, then code to pass them" (ATDD)
